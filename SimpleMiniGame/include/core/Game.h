...
public:
	...
	const std::shared_ptr<Entity>& getEntity(unsigned int idx) const;

	template <typename T>
	std::shared_ptr<T> buildEntityAt(const std::string& filename, int col, int row);

	void bigArray(std::vector<std::shared_ptr<System>> systems, float elapsed);
...
private:
	...
	// II.A A unique pointer of type Board 
	std::unique_ptr<Board> board;

	// III.D Declare an vector from the standard template library that 
	...
	std::vector<std::shared_ptr<System>> graphicSystem;
